...
LEFT JOIN blocked_users bu ON
	(bu.blocked_by_user_id = users_to_add.user_id AND bu.blocked_user_id = $4
	OR bu.blocked_by_user_id = $4 AND bu.blocked_user_id = users_to_add.user_id)
WHERE bu.blocked_by_user_id IS NULL
RETURNING user_id`

// there can be only two prepared statements for addUsersToRoom: allowGroupInvites and allowDirectMessages
...
// there can be only two prepared statements for getMessagesFromRoom: getOldMessages and getNewMessages
...
var getRoomUsers = `SELECT rui.user_id,rui.room_id,u.user_name,r.room_name,r.is_group,r.created_by_user_id
FROM room_users_info rui
JOIN users u ON rui.user_id = u.user_id
JOIN rooms r ON rui.room_id = r.room_id WHERE rui.room_id = $1 ORDER BY rui.user_id`
